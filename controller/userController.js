...
schedule.scheduleJob(cronSchedule, async function () {
  try {
    ...
    for (const user of findBirthdayUsers) {
      const followUserList = await Models.userFollowModel.find({ following: user._id });
      
      for (const followUser of followUserList) {
        const followerUser = await Models.userModel.findOne({ _id: followUser.follower });
        console.log("followerUser",followerUser)
        const senderDetail = await Models.userModel.findOne({ role: "admin" });

        const birthdayPersonName = user.name || `${user.firstName} ${user.lastName}`;
        const senderName = senderDetail.name || `${senderDetail.firstName} ${senderDetail.lastName}`;
  ...
  try {
    ...
    for (const eventIds of pastEvents){   
       ...
       for(let i=0;i<combinedArray.length-1;i++){
        ...
        const senderName = senderDetail.name || `${senderDetail.firstName} ${senderDetail.lastName}`;
        const senderProfile = senderDetail.profileImage || senderDetail.image;
        const reciverId=await Models.userModel.findOne({_id:combinedArray[i]})
...
module.exports = {
    ...
    deleteAccount: async (req, res) => {
        try {
          ...
          if(deletedUser){
            ...
            await Models.userFollowModel.deleteMany({follower:req.user._id,following:req.user._id});
            await Models.eventAttendesUserModel.deleteMany({userId:req.user._id})
            await Models.RSVPSubmission.deleteMany({userId:req.user._id})
            await Models.coHostModel.deleteOne({user_id:req.user._id})
            await Models.eventNotificationModel.deleteMany({senderId:req.user._id,reciverId:req.user._id})
          }
    ...
    getProfile: async (req, res) => {
        try {
          ...
          const allEvents = await Models.eventModel.find({})
          .populate({
            path: 'user',
          ...
          const attendEvents = await Models.eventAttendesUserModel.find({userId:req.user._id}).exec();

          const favourtieEvents = await Models.eventFavouriteUserModel.find({userId:req.user._id,favourite:1}).exec();
          ...
          const filteredPastEvents = filteredEventsPast.filter(event => {
            ...
            const eventDateTime = moment(endDate).add(Number(endTime.split(":")[0]), 'hours').add(Number(endTime.split(":")[1]), 'minutes');
            var time = Date.now();
            if(!endTimeStr){
              return false
            }
        ...
        // Now filteredPastEvents contains only past events
          ...
          let countTotalEventIAttended=await Models.eventAttendesUserModel.countDocuments({userId:req.user._id})
          let obj={};
   ...
   console.log("reqw.body",req.body)
      ...
      if(req.body.createRSVP&&req.body.createRSVP=="true"){
        ...
        if(req.body&&req.body.eventType=="private"||req.body.eventType=="public"){
      ...
      try {
        ...
        const sendNotifications = async (userIds, notificationTo) => {
          ...
          let c=  await Models.eventNotificationModel.updateMany( 
              ...
              { $set: { message: `You are invited as a ${notificationTo} for ${req.body.name} event`,notificationTo:notificationTo ,eventName:req.body.name} });
           }
        };
        let coHosts=(findEvent.coHosts.map((id)=>{ return id.toString()})).filter((id) => id !== req.user._id.toString());
        let guests=(findEvent.coHosts.map((id)=>{ return id.toString()})).filter((id) => id !== req.user._id.toString());
...
// Check event in past or not
    ...
    allAndVirtualEventAndNear: async(req,res)=>{
      try {
        ...
        if(type==1){
          ...
          countDocuments=await Models.eventModel.countDocuments({...baseQuery,...query,
    ...
    const FavouriteStatusMap = new Map();
      ...
      // Loop through each event in the result array
       result = result.map(event => {
           ...
           const deleteAccess =event.user? event.user._id.toString() === req.user._id.toString():false;

          // Check share access
          //  const shareAccess =event.guestsAllowFriend===true?(event.guests.some(guest => guest._id.toString() === req.user._id.toString())):false;
          const shareAccess=event.user?event.user._id.toString()===req.user._id.toString() ? true:(event.guestsAllowFriend===true?(event.guests.some(guest => guest._id.toString() === req.user._id.toString())):false):false;
    ...
    return helper.success(res, 'Success', {filteredPastEvents:filteredPastEvents,filteredPastEventsLength:countDocuments-count});
    ...
    myUpcomingEvents: async (req, res) => {
      try {
       ...
       rsvpEventIds.add(submission.eventId.toString()); // Add the eventId to the Set
        ...
        const favourtieEvents = await Models.eventFavouriteUserModel.find({userId:req.user._id,favourite:1})
        .populate({
          path: 'eventId',
          match: baseQuery, // Add your filter here
          populate: {
       ...
       let upcomingEvents1 = upcomingEventTest.map(event => {
  ...
  // });
    ...
    let countFollowing= await Models.userFollowModel.countDocuments({follower: req.params.id})
    let countTotalEventIAttended=await Models.eventAttendesUserModel.countDocuments({userId:req.params.id})
 ...
 //Terms and condition
    ...
    followStatusForAllUsersWithUserIdSartaj: async (req, res) => {
      try {
        ...
        const resultList = userList
          ...
          .filter(result => result.status === 'follow-each-other' ||result.status==="followed" || result.status === 'following'||result.status=="not-followed");
    ...
    clearNotification:async(req,res)=>{
      try {
        await Models.eventNotificationModel.deleteMany({reciverId:req.user._id});
        await Models.followingNotificationModel.deleteMany({reciverId:req.user._id});
   ...
   // Fetch event notifications for the logged-in user
    ...
    // Update event notifications with the rsvpForm property
     const notificationsWithRSVP = await Promise.all(eventNotifications.map(async(notification) => {
      let event=await Models.eventModel.findOne({_id:notification.eventId.toString()})
